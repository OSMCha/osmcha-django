...
class AreaOfInterestSerializer(GeoFeatureModelSerializer):
    ...
    class Meta:
        ...
        geo_field = "geometry"
        fields = ["id", "name", "filters", "geometry", "date", "changesets_url", "user"]
        validators = [
            UniqueTogetherValidator(
                queryset=AreaOfInterest.objects.all(), fields=("name", "user")
            )
        ]
...
class AreaOfInterestAnonymousSerializer(AreaOfInterestSerializer):
    ...
    def to_representation(self, instance):
        data = super(AreaOfInterestAnonymousSerializer, self).to_representation(
            instance
        )
