...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        print(f"Cloned osmcha-frontend ({repo.commit().hexsha}) to {temp_dir}")
        ...
        static_files = [
            join("css", f) for f in listdir(join(temp_dir, "static", "css"))
        ]
        static_files += [join("js", f) for f in listdir(join(temp_dir, "static", "js"))]
        static_files += [
            join("media", f) for f in listdir(join(temp_dir, "static", "media"))
        ]

        for file in static_files:
            shutil.copyfile(join(temp_dir, "static", file), join(static_dir, file))
